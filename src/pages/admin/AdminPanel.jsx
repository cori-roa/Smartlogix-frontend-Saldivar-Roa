    ...
    <div className="modal-overlay">
      <div className="modal modal-lg">
        ...
        <table className="data-table">
          <thead>
            <tr><th>SKU</th><th>Producto</th><th style={{ width: 160 }}>Cantidad en sucursal</th></tr>
          </thead>
          <tbody>
            {stocks.map(s => (
              <tr key={s.productoId}>
                ...
                <td>
                  <input type="number" className="form-input" value={s.cantidad} min="0"
                    onChange={e => set(s.productoId, e.target.value)}
    ...
    <div className="app-layout">
      <aside className="sidebar">
        ...
        <div className="sidebar-user">
          ...
          <div className="sidebar-user-pill">ADMINISTRADOR</div>
        </div>
                ...
                <div>
                  ...
                  <div className="stat-row mb-6" style={{ gridTemplateColumns: 'repeat(5, 1fr)' }}>
                    <div className="stat-cell">
                      ...
                      <div className="stat-value accent">{usuarios.length}</div>
                    </div>
                    <div className="stat-cell">
                    ...
                    <div className="stat-cell">
                      ...
                      <div className="stat-value">{sucursales.length}</div>
                    </div>
                    <div className="stat-cell">
                      <div className="stat-label">Stock bajo</div>
                      <div className="stat-value yellow">{sBajo}</div>
                    </div>
                    <div className="stat-cell">
                      <div className="stat-label">Sin stock</div>
                      <div className="stat-value red">{sSin}</div>
                    </div>
                  </div>
                        ...
                        : productos.filter(p => p.cantidad <= 5).slice(0, 5).map(p => (
                          <div key={p.id} className="list-item">
                            <div>
                              ...
                              <div className="list-item-secondary font-mono" style={{ fontFamily: 'monospace', fontSize: 11, color: 'var(--c-accent)' }}>{p.sku}</div>
                            </div>
                ...
                <div>
                  <div className="page-header flex justify-between items-center">
                    <div>
                      ...
                      <div className="page-desc">{usuarios.length} cuenta(s) registrada(s) en la plataforma</div>
                    </div>
                  ...
                  <div className="card">
                    <table className="data-table">
                      <thead>
                        <tr><th>Nombre</th><th>Correo</th><th>Cargo</th><th>Sucursal</th><th>Rol</th><th>Acciones</th></tr>
                      </thead>
                      <tbody>
                        {usuarios.map(u => (
                          <tr key={u.id}>
                            ...
                            <td>
                              {u.sucursalNombre
                                ? <span className="badge badge-blue">{u.sucursalNombre.replace('Sucursal ', '')}</span>
                            ...
                            <td>
                              <div className="flex gap-2">
                                <button className="btn btn-secondary btn-sm" onClick={() => { setUEdit(u); setModalU(true) }}>
                                  <IconEdit /> Editar
                                </button>
                                {u.email === usuario?.email
                                  ? <span className="badge badge-gray">Tu cuenta</span>
                                  : <button className="btn btn-danger-ghost btn-sm" onClick={() => eliminarUsuario(u.id)}>
                                      <IconTrash /> Eliminar
                                    </button>}
                              </div>
                            </td>
              ...
              {seccion === 'inventario' && (
                <div>
                  ...
                  <div className="card">
                    <table className="data-table">
                      <thead>
                        <tr><th>SKU</th><th>Nombre</th><th>Categoría</th><th>Precio</th><th>Stock global</th><th>Estado</th><th>Sucursales</th></tr>
                      </thead>
                      <tbody>
                        {productos.map(p => (
                          <tr key={p.id}>
                            <td><span className="font-mono" style={{ fontSize: 12, color: 'var(--c-accent)', fontFamily: 'monospace' }}>{p.sku}</span></td>
                            ...
                            <td>
                              <button className="btn btn-secondary btn-sm" onClick={() => { setProdStock(p); setModalStockProd(true) }}>
                                <IconMapPin size={12} /> Ver
                              </button>
                            </td>
                          </tr>
                        ))}
                      ...
                      return (
                        <div key={s.id} className="card">
                          <div className="card-header">
                            ...
                            <span className={`badge ${s.activa ? 'badge-green' : 'badge-gray'}`}>
                              <span className="badge-dot" />{s.activa ? 'Activa' : 'Inactiva'}
                            </span>
                          </div>
